...
**Latest commit:** `e67ea24` (Batch E). Working tree CLEAN. Everything through `e67ea24` builds Debug+Release green + smoke 40/40; the footage-confidence batch (A–E) is reviewer-cleared (no P0/P1). See §1 RESUME for the active goal + what remains (column-scroll P1 — align w/ Xavier first; VoiceOver P2).
...
**REMAINING / DEFERRED (then final-review → production-ready):**
- **Column scroll for many-card overflow** (auditor P1) — STILL DEFERRED, design-sensitive: interacts with the funnel anchors / `destFrames` / the centering spacers; a `SmoothScrollView` exists at ~9923. Rare (funnel caps at 6-8). This is the one item that can regress the ring/funnel geometry → **align with Xavier before touching** (per the design-heavy rule).
- **VoiceOver-label pass** on icon-only buttons (P2) — broad but safe/additive; do as its own focused batch.
- verify-wording ("spot-checked" vs actual SHA-256 — LOW; `statusText` isn't shown in v3 anymore except via the toast).
- Then: final reviewer pass on any new work, build Debug+Release + `./smoke_test.sh` + unit (`⌘U` / the `pkill testmanagerd`+settle trick), and certify production-ready.
