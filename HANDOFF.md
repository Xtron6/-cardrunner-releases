...
**Latest commit:** `7095fe6` ("Polish: airtight VERIFIED badge + column scroll for card overflow"). Working tree CLEAN.

**🚢 SHIPPED — CardRunner 1.8.0 (build 21) released via Sparkle on 2026-07-02.** The whole `nway-rebuild` line (footage-confidence A–F + 6 hardware fixes + Swift-6 cleanup + VoiceOver) is now LIVE to users — VERIFIED live (appcast serves 1.8.0/build 21, DMG returns HTTP 200). Release pipeline: `./release.sh 1.8.0 "<pipe-separated notes>"` → smoke 40/40 + unit 68/68 + Release archive → DMG → **notarize → staple → EdDSA sign_update → appcast publish**.
- **The appcast users read is in a SEPARATE repo** `Xtron6/-cardrunner-releases` (release.sh pushes it there → `https://raw.githubusercontent.com/Xtron6/-cardrunner-releases/refs/heads/main/appcast.xml`); the `appcast.xml` in THIS repo is just a mirror. DMGs upload to that repo's GitHub Releases.
- **Notarization gotcha (cost ~30 min this ship):** Apple pushed an updated **Developer Program License Agreement** (~June/WWDC) that 403-blocked notarization ("a required agreement is missing or has expired") until the **Account Holder** accepted it at developer.apple.com/account. NOT a code/credentials issue — `xcrun notarytool history --keychain-profile CardRunner-Notary` kept working, proving it's the legal gate. Test the gate without a full rebuild via `xcrun notarytool submit CardRunner-X.Y.Z.dmg --keychain-profile CardRunner-Notary --wait`.
- **Dev-machine note:** wiping `~/Library/Developer/Xcode/DerivedData/CardRunner-*` also removes the resolved SwiftPM checkouts (Sparkle 2.9.1 / Sentry 8.58.1); if a build then fails on "Package.swift was modified during the build", it's a resolve race with an open Xcode — retry `xcodebuild -resolvePackageDependencies` (or Xcode ▸ File ▸ Packages ▸ Reset Package Caches).
- Next release must bump `CURRENT_PROJECT_VERSION` above **21**. Hardware happy-path (new footage copies clean → VERIFIED → safe-to-pull) was validated by Xavier before ship.
- **POST-SHIP REAL-WORLD SESSION (2026-07-07):** Two-card shoot (1,157 photos 210 MB/s + 109 video clips 132 MB/s) ran clean. Bugs found and fixed — see commits `ca5909b` + `7095fe6` below. Both former deferred items (column-scroll P1, VERIFIED-badge P2) are now DONE.

**★ POST-SHIP POLISH BATCH (2026-07-07, `ca5909b` + `7095fe6`, reviewer no P0/P1):** Real-world two-card shoot surfaced 4 bugs + 2 deferred polish items, all now fixed.

- **`ca5909b` — Bug fixes: F/Reveal path, ring Open in Finder, wrong-mode history, rename chip**
  - **F key + history Reveal** now land in the exact `clips/260707/cardname` subfolder. Shell already emitted `FOLDERSYNC_START dest=<full/path>` but Swift never parsed it — now `applyIngestProgressLine` handles it and sets `destPath` early in the copy phase. `PROGRESS_DEST` no longer overwrites if `FOLDERSYNC_START` already set a more specific path (used only for 0-new fallback).
  - **"Open in Finder" button was unreachable** in the ring after transfer. `v3RingCenter` checked `!v3AnyActive` before `v3AllDone` — done-phase cards are excluded from `v3ActiveWork`, so `!v3AnyActive` fired first showing "Ready for cards" instead of the green completion screen. Swapped condition order.
  - **Wrong-mode history entries** are now amber (`exclamationmark.circle`, "Wrong mode" badge, amber skip text, subtle amber tinted row) instead of neutral gray. Gated on `skipWrongMode > 0 && newFiles == 0`; partial-success runs stay green.
  - **Mid-transfer rename** can't be made truly instant (cardcopy writes to a fixed dir path per batch; renaming mid-write causes ENOENT). Post-completion rename unchanged. Added amber "Renaming to X after copy" chip in the active lane so the operator knows it's queued.
  - **Empty `260702/photos` folder** on the destination — NOT a bug from this session. Created 2026-07-02 10:51 AM (pre-existing). Safe to delete manually.

- **`7095fe6` — Polish: airtight VERIFIED badge + column scroll**
  - **VERIFIED badge (was P2):** `ActiveIngest.verifyEnabled` field added — snapshots `verifyTransfer || fullVerifyEnabled` at ingest launch. Badge, done-phase display string, and status text all use the snapshot; immune to mid-transfer toggle changes.
  - **Column scroll (was P1):** `v3Sources` wrapped in `GeometryReader` + `ScrollView(.vertical, showsIndicators: false)`. 1–4 cards: centering Spacers preserved via `minHeight: max(0, geo.size.height - 170)` (bias math folded into min-height, replacing old `Color.clear.frame(height:170)` trick — identical visual result, zero dead scroll space on overflow). 6+ cards: VStack grows to content height and scrolls. `anchorPreference` values for funnel connectors propagate correctly through scroll views in SwiftUI window coordinates.
...
- ~~**Column scroll for many-card overflow**~~ **✅ DONE `7095fe6`** — `GeometryReader` + `ScrollView` wraps `v3Sources`; centering Spacers preserved via `minHeight` math.
- ~~**Airtight VERIFIED badge**~~ **✅ DONE `7095fe6`** — `ActiveIngest.verifyEnabled` snapshots `verifyTransfer||fullVerifyEnabled` at launch; badge and status string use snapshot.
- verify-wording ("spot-checked" vs actual SHA-256 — LOW; `statusText` isn't shown in v3 anymore except via the toast).

**⚠️ STILL XAVIER'S TO VALIDATE (automated review + smoke can't cover):** run **68/68 unit tests via Xcode ⌘U** (the CLI host is environmentally flaky — hand this to Xavier), plus a hardware pass of the new signals on a REAL card: a completed card sits in the green pile until ejected; ✓VERIFIED + "N files·duration·MB/s" reconciliation; mirror-confirmation with a 2nd drive; low-disk + OFFLINE badges; a no-destination/unmounted-primary card PARKS + toasts instead of vanishing; VoiceOver reads the icon buttons. **Also validate the 2026-07-07 fixes:** F key lands in `clips/date/cardname`; ring shows green "All safe to pull" + "Open in Finder" after completion; wrong-mode history rows are amber; column scrolls cleanly with 5+ DEV fixture cards.
