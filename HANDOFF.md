...
**Latest commit:** `f6166e7` ("appcast: record 1.8.0 (build 21) release entry"). Working tree CLEAN. Post-ship housekeeping done: appcast.xml committed to this repo, `*.dmg` added to `.gitignore` (release DMGs no longer clutter `git status`), stray Spotlight dupes cleared. (The local `CardRunner-1.8.0.dmg` remains on disk but is now gitignored.)
...
- Next release must bump `CURRENT_PROJECT_VERSION` above **21**. Hardware happy-path (new footage copies clean → VERIFIED → safe-to-pull) was validated by Xavier before ship. Still-deferred, NON-blocking: column-scroll P1 (design-sensitive, align w/ Xavier first), airtight-VERIFIED-badge P2 (chip `task_e836910d`).
